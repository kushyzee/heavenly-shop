...
const cartReducer = (state, action) => {
  ...
  switch (type) {
    case "ADD_TO_CART":
      ...
      if (existingItem) {
        const updatedCart = state.cart.map(item => item.id === payload.id ? {...item, quantity: item.quantity + 1} : item)
    ...
    case "REMOVE_FROM_CART":
      ...
      const updatedSubtotalAfterRemoval = state.subtotal - (itemToRemove.koboPrice * itemToRemove.quantity);
    ...
    case 'INCREASE_QUANTITY':
      const increasedCart = state.cart.map(item => item.id === payload ? {...item, quantity: item.quantity + 1} : item)
      const increasedItem = state.cart.find(item => item.id === payload)
      const updatedSubtotalAfterIncrease = state.subtotal + increasedItem.koboPrice
      ...
      const decreasedCart = state.cart.map(item => item.id === payload && item.quantity > 1 ? {...item, quantity: item.quantity - 1} : item)
      const decreasedItem = state.cart.find(item => item.id === payload)
      const updatedSubtotalAfterDecrease = state.subtotal - decreasedItem.koboPrice
